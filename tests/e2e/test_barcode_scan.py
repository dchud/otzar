...
class TestBarcodeScanPage:
    ...
    def test_scan_page_has_camera_controls(
...
class TestPhoneScannerMode:
    ...
    def test_phone_mode_shows_phone_scanner_title(
        self, page, live_server, staff_user
    ):
    ...
    def test_phone_scan_sends_to_queue(
        ...
        expect(
            page.locator("text=Sent ISBN 0875847625 to review")
        ).to_be_visible()
...
class TestQRCodeHandoff:
    ...
    def test_qr_code_requires_auth(self, page, live_server, db):
        ...
        response = page.request.get(
            f"{live_server.url}/ingest/qr/", max_redirects=0
        )
        assert response.status == 302

    def test_phone_auth_logs_in_and_redirects(
        self, page, live_server, staff_user
    ):
    ...
    def test_phone_auth_expired_token(self, page, live_server, staff_user):
        ...
        with patch(
            ...
            response = page.request.get(
                f"{live_server.url}/ingest/phone-auth/{token}/"
            )
    ...
    def test_phone_auth_bad_token(self, page, live_server, db):
        ...
        response = page.request.get(
            f"{live_server.url}/ingest/phone-auth/bad-token/"
        )
