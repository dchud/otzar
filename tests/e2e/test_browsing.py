...
class TestBrowseViews:
    ...
    def test_author_browse_shows_authors(
        self, page, live_server, sample_record
    ):
        page.goto(f"{live_server.url}/browse/authors/")
        expect(
            page.get_by_role("link", name="Brown, John Seely")
        ).to_be_visible()

    def test_author_browse_is_clickable(
        self, page, live_server, sample_record
    ):
        page.goto(f"{live_server.url}/browse/authors/")
    ...
    def test_publisher_browse_is_clickable(
        self, page, live_server, sample_record
    ):
        page.goto(f"{live_server.url}/browse/publishers/")
        expect(
            page.locator("text=Harvard Business School Press")
        ).to_be_visible()

    def test_location_browse_is_clickable(
        self, page, live_server, sample_record
    ):
    ...
    def test_browse_index_has_places_link(
        self, page, live_server, sample_record
    ):
        page.goto(f"{live_server.url}/browse/")
...
class TestRecordDetail:
    def test_record_detail_shows_all_fields(
        self, page, live_server, sample_record
    ):
        page.goto(
        ...
        expect(
            page.get_by_role("link", name="Brown, John Seely").first
        ).to_be_visible()
        expect(
            page.get_by_role("link", name="Duguid, Paul").first
        ).to_be_visible()
        # Subjects
        expect(page.locator("text=Information society")).to_be_visible()
        # Identifiers
