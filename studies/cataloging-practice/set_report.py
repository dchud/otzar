...
TITLE = {
    work: (re.compile("^" + PREFIX + pattern.lstrip("^")),
           re.compile(pattern.lstrip("^")))
    for work, pattern in PATTERNS.items()
}


def names_work(work, r):
    """Whether a described record's 245 names the work: the title filter."""
    rx, rx_b = TITLE[work]
    return bool(rx.search(norm(r["title"]))
                or (r.get("title_b") and rx_b.search(norm(r["title_b"]))))


def cell(work, recs):
    """The counts for one work in one catalog's response."""
    named = [r for r in recs if names_work(work, r)]
...
def main():
    ...
    for work, cats in data.items():
        ...
        for c in CATALOGS:
            ...
            s = summary[work][c] = cell(work, recs)
            dropped[f"{c}:title"] += s["returned"] - s["named"]
            dropped[f"{c}:not-monograph"] += s["named"] - s["monograph"]
    ...
    for c in CATALOGS:
        ...
        print(f"{c:9}{r['library']:9}{r['l19_a']:7}{r['l19_b']:7}"
              f"{r['l19_c']:7}{r['t773']:6}{r['np']:6}"
              f"{r['set_level_extent']:9}{r['t505']:6}"
              f"{r['vendor_online']:8}")
        tot[c] = r["library"]
    ...
    print(f"\nleader/19 coded at LC + Oxford + NLI: {anglo} of {anglo_n} "
          f"records")
    ...
    with out.open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        cols = ["returned", "named", "monograph", "vendor_online",
                "library", "l19_a", "l19_b", "l19_c", "t773", "np", "t505",
                "extent_counted", "extent_open", "set_level_extent",
                "truncated"]
